...
Function SetUpJava
	...
	${If} ${FileExists} "$PortableAppsPath\CommonFiles\Java\JavaPortable.ini"
;		StrCpy $FixJava "true"
;		ReadINIStr $JavaVer "$PortableAppsPath\CommonFiles\Java\JavaPortable.ini" JavaPortable Version
;
;		ReadRegStr $OldJavaVer HKLM "SOFTWARE\JavaSoft\Java Runtime Environment" "BrowserJavaVersion"
;		WriteRegStr HKLM "SOFTWARE\JavaSoft\Java Runtime Environment" "BrowserJavaVersion" $JavaVer
;
;		ReadRegStr $OldJavaPath HKLM "SOFTWARE\JavaSoft\Java Runtime Environment\$JavaVer" "JavaHome"
;		WriteRegStr HKLM "SOFTWARE\JavaSoft\Java Runtime Environment\$JavaVer" "JavaHome" "$PortableAppsPath\CommonFiles\Java"
;
;		ReadRegStr $OldJavaPath2 HKLM "SOFTWARE\JavaSoft\Java Plug-in\$JavaVer" "JavaHome"
;		WriteRegStr HKLM "SOFTWARE\JavaSoft\Java Plug-in\$JavaVer" "JavaHome" "$PortableAppsPath\CommonFiles\Java"

		${If} $IMPORTJAVA != "false"
			${IfNot} ${FileExists} "$PROGRAMDIRECTORY\Plugins\npjp*.dll"
				${IfNot} ${FileExists} "$PROGRAMDIRECTORY\Plugins"
					CreateDirectory "$PROGRAMDIRECTORY\Plugins"
				${EndIf}

				${If} ${FileExists} "$PortableAppsPath\CommonFiles\Java\bin\new_plugin"
					CopyFiles /SILENT "$PortableAppsPath\CommonFiles\Java\bin\new_plugin\*" "$PROGRAMDIRECTORY\Plugins\"
				${Else}
					CopyFiles /SILENT "$PortableAppsPath\CommonFiles\Java\bin\np*.dll" "$PROGRAMDIRECTORY\Plugins\"
					CopyFiles /SILENT "$PortableAppsPath\CommonFiles\Java\bin\msvcr71.dll" "$PROGRAMDIRECTORY\Plugins\"
				${EndIf}
			${EndIf}
		${EndIf}
	${EndIf}
...
!endif
	...
	${If} $SECONDARYLAUNCH != "true"
		...
		ClearErrors
		ReadRegStr $0 HKCU "Software\Google\Update\ClientState\${CLSID}" ""
		${If} ${Errors}
		${AndIf} $1 == ""
			StrCpy $3 "False"
		${Else}
...
!ifdef CHROMESXSMODE
		ClearErrors
		ReadRegStr $2 HKCU "Software\Google\Update\ClientState\${CLSIDSXS}" ""
		${If} ${Errors}
		${AndIf} $1 == ""
			StrCpy $4 "False"
		${Else}
